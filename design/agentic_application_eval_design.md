...
```
1.  JD Decomposition            → jd_components.json
2.  Form Decomposition          → form_questions.json
3.  Raw Context Capture         → raw_context.md
4.  Context Mapping             → tagged_context.json (+ tagged_context.md)
5.  Reflective Interview        → interview_report.md
6.  Preferences File            → preferences.md
7.  Voice Profiling             → voice_profile.md
8.  CV Tailoring                → cv_tailored.md + cv_tailoring_notes.json
9.  Draft Generation            → drafts/genN/vXX/q<question_id>.md + vXX.meta.json
10. Judge Panel Evaluation      → evals/genN/vXX_<judge>.json
11. Aggregation                 → evals/genN/summary.json + summary.md
12. Human Sniff Check           → rounds/genN/direction.md (manual, not scripted)
13. Directed Iteration          → rounds/gen(N+1)/round_config.json
```

Each stage is a separate script/module, callable independently, so you can
re-run stage 10 alone if you tweak the judge panel without regenerating
drafts.
...
**Output:** `jd_components.json` — a structured breakdown, e.g.:
...
Done once by an LLM call against the raw JD text, then **reviewed and
hand-edited by Jeremy** — a wrong or sloppy decomposition poisons every
downstream score. These four top-level keys (`required_qualifications`,
`preferred_qualifications`, `core_responsibilities`, `mission_signals`) are
also the grouping used for the section-level rollup in Stage 11.

---

### Stage 2 — Form Decomposition

**Input:** the raw text of the actual application form (the employer's
application portal, not the job-description posting — these are different
documents; for the Anthropic application in progress, the JD is confirmed
but the form's actual question count is not).

**Output:** `form_questions.json` — purely structural facts about the form
itself. Deliberately does **not** pre-assign JD components to a question;
that mapping happens at generation time (Stage 9), not here.
...
reference unambiguous and lets Stages 10–11 glob by question id.
...
### Stage 4 — Context Mapping

**Input:** `raw_context.md`, `jd_components.json`, `form_questions.json`.

**Output:** `tagged_context.json` — the canonical, machine-generated
mapping of fragments of the raw dump onto JD components and (once
available) form questions, with a confidence field per fragment. A
...
`coverage_gaps` is computed here but does not gate Stage 5 — it's
informational input to it, not a precondition.

---

### Stage 5 — Reflective Interview
...
of whether Stage 4 found coverage gaps. Its job is broader: a curious,
...
`tagged_context.json` (including coverage gaps, as a hint toward
under-explored territory, not a script to follow), and the CV.
...
first-class input to Stage 9, distinct from `tagged_context.json` — it is
not merged back into the tagged-context file.

**Open note:** whether these verbatim quotes count as "corrected" text for
Stage 7's sourcing rule, or need a light grammar pass first (they may
...
`rounds/genN/direction.md` (Stage 12), not here, so this file doesn't
...
Stage 9 generation call, not regenerated per variant or per round.

**Guardrail for the judge panel:** a distinct, low-weight `style_fidelity`
check (see Stage 10), separate from content-quality scores, catches drift
...
back to a specific context fragment or flags it as unsupported:
...
essay's judge-scored `overclaim_risk` (Stage 10), for one consistent
audit trail across both artifacts.

---

### Stage 9 — Draft Generation

**Input:** `jd_components.json`, `tagged_context.json`,
`interview_report.md`, `form_questions.json`, `preferences.md`,
`voice_profile.md`, and the current round's `rounds/genN/round_config.json`
(mode + variant count + carried-forward direction from the prior round, if
any — see Stage 13).
...
- **Exploration mode** — produces the full variant set (default 5–10
  ...
  goal of reducing his own time agonizing over text). Feeds Stage 10.
- **Convergence mode** — produces a single refined draft bundle directly,
  ...
  tournament; Stage 10 is skipped entirely for that round.
...
### Stage 10 — Judge Panel Evaluation
...
just per section) — this is still what drives Stage 13's decision about
which components need work; the section-level view (required/preferred/
responsibilities/mission) is a Stage 11 rollup for readability, not how
...
### Stage 12 — Human Sniff Check + Direction Capture
...
scores moved. `input/` stays gitignored regardless (raw personal material);
everything else here is intended to be committed and public.
...
no panel) — chosen by Jeremy at Stage 12, not defaulted.
...
- The corrected, accurate framing of the Kiribati work (no formal model
  ...
  (judge-scored) and the CV (`claims_checklist`, Stage 8).
...
- Build `voice_profile.md` (Stage 7) only from corrected source text —
  ...
  `interview_report.md`'s verbatim quotes (Stage 5) count as "corrected"
  for this rule, or need a light grammar pass first — unresolved, decide
  before Stage 7 is implemented against real interview output.
